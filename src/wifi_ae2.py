...
X_all_train = np.empty(shape=[0,4])
X_all_train=np.empty(shape=[0,4])
...
for item in path_read:
    ...
    if total_num != 0:
        for i in range(total_num):
            ...
            # venv\data\indoorA305/10/10_14/
            # venv\data\indoorA305/1/1_1/
            # venv\data\indoorA305/1/1_14/
            # venv\data\indoorA305/1/1_7/
            # venv\data\indoorA305/7/7_10/
            # venv\data\indoorA305/10/10_7/
            #                          25
            length = len(item)
            csi_data = np.c_[csi_len, csi_ang]
            length = len(item)
            if (item[length - 10] == '1' or item[length - 10] == '2') and item[length - 9] == '/' and item[
                length - 11] == '_':
                X_ae_test = np.append(X_ae_test, csi_data, axis=0)
                X_rss_test = np.append(X_rss_test, rss, axis=0)
                y_true_test[kk] = -1
                kk += 1
            #elif '7' <= item[length - 10] <= '9' and ('4' <=item[length - 12]<='7'):# or item[length - 12]==0) :
            # elif item[length - 10] == '8':
            # 右一小块
            # 中间一小块
            # 8
            # 7-9
            # 5-11
            # 3-14
            #elif '7' <= item[length - 10]<='9' and ('7' <=item[length - 12]<='9' or (item[length - 12]==0)): # :
            # elif '5' <= item[length - 10] <= '9' or (item[length-11]=='1' and '0'<= item[length-10]<='1'):
            # #     # '3' <= item[length - 10] <= '13'  q4
...
for epoch in range(EPOCH):
    ...
    for i, (x,) in enumerate(train_loader):
       # ans = x.shape
        ans=x[0]
        encoded,decoded = autoencoder(ans)
        loss = loss_func(decoded, ans)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()

        total_loss += loss.item() * len(x)
    total_loss /= len(X_ae_train)
